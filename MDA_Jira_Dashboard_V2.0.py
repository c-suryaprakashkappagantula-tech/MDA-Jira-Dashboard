...
# -*- coding: utf-8 -*-
# MDA_Jira_Dashboard_V2.0.py  —  SELF-CONTAINED (no external .py dependencies)
...
_QTR_HEADLESS = os.getenv('HEADLESS', 'true').lower() == 'true'
...
def run_qmetry_test_report(iteration):
    with sync_playwright() as p:
        browser = p.chromium.launch(headless=_QTR_HEADLESS, channel='msedge')
        context = browser.new_context(accept_downloads=True)
        page = context.new_page()
        try:
            ...
            _qtr_info(f'[{_qtr_tnow()} ms] === QMetry Test Report module completed ===')
            _qtr_info(f'Downloaded CSV: {csv_path}')
            _qtr_info(f'Generated Excel: {excel_path}')
            return excel_path
        finally:
...
_S2_TEMPLATE_PATH = BASE_DIR / 'templates' / 'MDA QA INTG_Slide2.pptx'
...
with left_col:
    ...
    btn_col1, btn_col2, btn_col3 = st.columns([1.2, 0.6, 0.6])
    with btn_col1:
        ...
        with _redir(logger):
            try:
                if m1 or m2:
                    with sync_playwright() as p:
                        ...
                        if m1:
                            ...
                            labels_pivot = run_qmetry_automation(browser, iteration, integration)
                            print('[DEBUG] run_qmetry returned: %r (type=%s)' % (labels_pivot, type(labels_pivot).__name__))
                            if labels_pivot:
                                results['labels_pivot'] = Path(str(labels_pivot))
                                print('[OK] Labels pivot => %s (exists=%s)' % (results['labels_pivot'], results['labels_pivot'].exists()))
                            else:
                                results['labels_pivot'] = None
                                print('[WARN] QMetry returned None — no pivot created')
                        if m2:
                            logger.set('\U0001F41E Jira Defect Extract + Pivot (Defects) ...')
                            print('[STEP] Running Jira Defect Extract + Pivot...')
                            defects_pivot = run_defect_automation(browser, iteration, integration)
                            print('[DEBUG] run_defect returned: %r (type=%s)' % (defects_pivot, type(defects_pivot).__name__))
                            if defects_pivot:
                                results['defects_pivot'] = Path(str(defects_pivot))
                                print('[OK] Defects pivot => %s (exists=%s)' % (results['defects_pivot'], results['defects_pivot'].exists()))
                            else:
                                results['defects_pivot'] = None
                                print('[WARN] Defect returned None — no pivot created')
                        browser.close()

                if m3:
                    logger.set('\U0001F4C8 Jira -> Test Execution Report + Excel ...')
                    print('[STEP] Running Jira -> Test Execution Report (embedded)...')
                    excel_path = run_qmetry_test_report(iteration)
                    print('[DEBUG] run_qmetry_test_report returned: %r (type=%s)' % (excel_path, type(excel_path).__name__))
                    if excel_path:
                        results['exec_excel'] = Path(str(excel_path))
                        print('[OK] Generated Excel => %s (exists=%s)' % (results['exec_excel'], results['exec_excel'].exists()))
                    else:
                        results['exec_excel'] = None
                        print('[WARN] Test report returned None — no excel created')
                ...
                if m1 and m2 and m3:
                    ...
                    if missing:
                        detail = '; '.join(missing)
                        print('[ERROR] PPT blocked — %s' % detail)
                        raise RuntimeError('PPT blocked: %s' % detail)
                    labels_df = pd.read_excel(lp)
                    defects_df = pd.read_excel(dp)
                    qtr_df = pd.read_excel(xp)
...
st.markdown("<div style='text-align:center;padding:12px 0;color:#b8c0cc;font-size:11px;font-weight:500;letter-spacing:0.5px;'>"
            "MDA Jira Dashboard &bull; V2.0 &bull; Electric Teal</div>", unsafe_allow_html=True)
