...
# -*- coding: utf-8 -*-
# MDA_Jira_Dashboard_V5.0.py  —  SELF-CONTAINED (no external .py dependencies)
...
# V4.0: Vendor filter modes
VENDOR_MODE_ALL = 'All Vendors'
VENDOR_MODE_INFY = 'Infy'
VENDOR_MODE_OTHER = 'Other Vendors'
INFY_ASSIGNEES = list(ALLOWED_ASSIGNEES)  # QMetry user IDs
INFY_REPORTERS = list(ALLOWED_REPORTERS)  # Jira display names
...
def filter_qmetry_data(excel_file: Path, vendor_mode=VENDOR_MODE_INFY) -> Path:
    df = pd.read_excel(excel_file)
    original_count = len(df)
    if 'Assignee' in df.columns:
        if vendor_mode == VENDOR_MODE_ALL:
            df_filtered = df.copy()
            df_filtered['Assignee'] = df_filtered['Assignee'].map(NAME_MAPPING).fillna(df_filtered['Assignee'])
            print(f'QMetry (All Vendors): {original_count} test cases (no filter)')
        elif vendor_mode == VENDOR_MODE_OTHER:
            df_filtered = df[~df['Assignee'].isin(INFY_ASSIGNEES)]
            print(f'QMetry (Other Vendors): {original_count} -> {len(df_filtered)} test cases')
        else:  # VENDOR_MODE_INFY
            df_filtered = df[df['Assignee'].isin(INFY_ASSIGNEES)]
            df_filtered['Assignee'] = df_filtered['Assignee'].map(NAME_MAPPING).fillna(df_filtered['Assignee'])
            print(f'QMetry (Infy): {original_count} -> {len(df_filtered)} test cases')
...
def filter_defect_data(excel_file: Path, vendor_mode=VENDOR_MODE_INFY) -> Path:
    df = _ca_read_defect_excel(excel_file)
    original_count = len(df)
    if 'Reporter' in df.columns:
        if vendor_mode == VENDOR_MODE_ALL:
            df_filtered = df.copy()
            print(f'Defects (All Vendors): {original_count} defects (no filter)')
        elif vendor_mode == VENDOR_MODE_OTHER:
            df_filtered = df[~df['Reporter'].isin(INFY_REPORTERS)]
            print(f'Defects (Other Vendors): {original_count} -> {len(df_filtered)} defects')
        else:  # VENDOR_MODE_INFY
            df_filtered = df[df['Reporter'].isin(INFY_REPORTERS)]
            print(f'Defects (Infy): {original_count} -> {len(df_filtered)} defects')
...
def run_qmetry_automation(browser, iteration: str, integration_name: str, vendor_mode=VENDOR_MODE_INFY):
    ...
    try:
        ...
        filtered_path = filter_qmetry_data(download_path, vendor_mode=vendor_mode)
...
def run_defect_automation(browser, iteration: str, integration_name: str, vendor_mode=VENDOR_MODE_INFY, label_type='PROGRESSION'):
    context = browser.new_context(accept_downloads=True)
    context.tracing.start(screenshots=True, snapshots=True)
    page = context.new_page()
    ...
    try:
        ...
        search_query = f'project = "WPATEST-Integration Test Team" AND issuetype = Defect AND labels = INTG_UAT_{label_type}_{iteration} and "Service Type." = T-Mobile'
        ...
        download_path = ARTIFACTS / f'Jira_defects_{iteration}_{label_type}.xlsx'
        ...
        filtered_path = filter_defect_data(download_path, vendor_mode=VENDOR_MODE_ALL if label_type == 'REGRESSION' else vendor_mode)
...
def _qtr_post_process_csv_to_excel(csv_path, iteration_str, vendor_mode=VENDOR_MODE_INFY):
    ...
    if exec_col is None:
        _qtr_info(f"[WARN] 'Execution Assignee' column not found. Skipping filter.")
        df_f = df.copy()
    elif is_regression or vendor_mode == VENDOR_MODE_ALL:
        df_f = df.copy()
        _qtr_info(f'[{_qtr_tnow()} ms] No assignee filter (mode={vendor_mode}, regression={is_regression}): {len(df_f)} rows')
    elif vendor_mode == VENDOR_MODE_OTHER:
        ser = df[exec_col].astype(str).str.strip()
        df_f = df[~ser.isin(INFY_REPORTERS)].copy()
        _qtr_info(f'[{_qtr_tnow()} ms] Other Vendors filter: kept {len(df_f)} of {len(df)} rows')
    else:  # VENDOR_MODE_INFY
        ser = df[exec_col].astype(str).str.strip()
        df_f = df[ser.isin(INFY_REPORTERS)].copy()
        _qtr_info(f'[{_qtr_tnow()} ms] Infy filter: kept {len(df_f)} of {len(df)} rows')
...
def run_qmetry_test_report(iteration, keep_browser=False, vendor_mode=VENDOR_MODE_INFY):
    if keep_browser:
    ...
    try:
        ...
        try:
            ...
            excel_path = _qtr_post_process_csv_to_excel(csv_path, iteration, vendor_mode=vendor_mode)
        ...
        if reg_key:
            ...
            try:
                ...
                reg_excel_path = _qtr_post_process_csv_to_excel(reg_csv_path, f'{iteration}_REG', vendor_mode=vendor_mode)
...
def _s2_populate_defect_table(slide, defects_df, iteration, reg_defects_df=None):
    ...
    reg_totals = _s2_build_priority_totals(reg_defects_df) if reg_defects_df is not None and not reg_defects_df.empty else None

    row_labels = [
        (f'Integration progression {cur_iter}', p_totals),
        (f'Integration  Regression {cur_iter}', reg_totals),
        (f'Automation Regression {major} X', None),
        ('Smoke testing', None),
    ]
    for ri, (label, totals_data) in enumerate(row_labels, start=1):
        if ri >= len(tbl.rows): break
        _s2_clear_and_set_cell(tbl, ri, 0, label)
        if totals_data:
            grand = 0
            for p_key in ['P0', 'P1', 'P2', 'P3', 'P4']:
                ci = header_map.get(p_key); val = totals_data.get(p_key, 0)
...
def add_slide2_delivery_updates(dst_prs, iteration, labels_df=None, defects_df=None,
                                exec_df=None, reg_exec_df=None, reg_defects_df=None,
                                template_path=_S2_TEMPLATE_PATH, insert_index=1):
    ...
    if defects_df is not None:
        _s2_populate_defect_table(new_slide, defects_df, iteration, reg_defects_df=reg_defects_df)
...
st.markdown(
    ...
    @keyframes rainbowGlow { 0% { box-shadow: 0 0 15px #f43f5e40; } 25% { box-shadow: 0 0 15px #f59e0b40; } 50% { box-shadow: 0 0 15px #10b98140; } 75% { box-shadow: 0 0 15px #3b82f640; } 100% { box-shadow: 0 0 15px #8b5cf640; } }

    .stApp {
        background: linear-gradient(135deg, #0f0c29 0%, #1a1040 30%, #24243e 60%, #0f0c29 100%) !important;
    ...
    .block-container { padding-top: 0 !important; padding-left: 130px !important; padding-right: 24px !important; max-width: 100% !important; }
    ...
    .left-rail {
        ...
        animation: fadeSlideLeft 0.5s ease-out, glow 4s ease-in-out infinite;
    }
    .rail-spacer { flex: 1; }
    ...
    .title {
        ...
        background: linear-gradient(135deg, #fff 0%, #a78bfa 40%, #60a5fa 70%, #34d399 100%);
    ...
    .pill:hover { background: rgba(139,92,246,0.25); transform: translateY(-2px); box-shadow: 0 6px 20px rgba(139,92,246,0.3); }
    ...
    .stat::before { content: ""; position: absolute; top: 0; left: 0; right: 0; height: 3px; background: linear-gradient(90deg, var(--accent-start), var(--accent-end)); }
    .stat:hover { transform: translateY(-6px) scale(1.02); box-shadow: 0 12px 36px rgba(0,0,0,0.4), 0 0 25px var(--accent-start); }
    .stat-purple { --accent-start: #8b5cf6; --accent-end: #c4b5fd; }
    .stat-blue { --accent-start: #3b82f6; --accent-end: #93c5fd; }
    .stat-rose { --accent-start: #f43f5e; --accent-end: #fda4af; }
    .stat-amber { --accent-start: #f59e0b; --accent-end: #fcd34d; }
    .stat .label { font-size: 11px; color: #a0a8b8; font-weight: 700; text-transform: uppercase; letter-spacing: 1px; }
    .stat .value { font-size: 34px; font-weight: 900; color: #FFFFFF; margin-top: 6px; text-shadow: 0 0 20px rgba(139,92,246,0.15); }
    ...
    .cli-header {
        ...
        border-radius: 14px; padding: 10px 14px; margin: 0 0 10px 0;
        display: flex; align-items: center; gap: 8px;
    }
    .cli-header::before { content: "●"; color: #a78bfa; font-size: 10px; animation: pulse 2s infinite; }
    .cli-box {
    ...
    .cli-box::-webkit-scrollbar-track { background: rgba(255,255,255,0.03); border-radius: 3px; }
    .cli-box::-webkit-scrollbar-thumb { background: rgba(139,92,246,0.35); border-radius: 3px; }

    /* ═══ BUTTONS — gradient glow ═══ */
    .stButton > button {
        background: linear-gradient(135deg, #7c3aed 0%, #3b82f6 50%, #10b981 100%) !important;
        background-size: 200% auto !important;
        color: #FFFFFF !important; border: 0 !important;
        padding: 0.85rem 1.5rem !important; border-radius: 16px !important;
        font-weight: 700 !important; font-size: 14px !important;
        box-shadow: 0 6px 24px rgba(124,58,237,0.3) !important;
        transition: all 0.3s cubic-bezier(0.4,0,0.2,1) !important;
        animation: rainbowGlow 3s ease infinite !important;
    }
    .stButton > button:hover {
        background-position: right center !important;
        transform: translateY(-3px) scale(1.02) !important;
        box-shadow: 0 10px 35px rgba(124,58,237,0.45) !important;
    }
    ...
    [data-testid="stRadio"] span, [data-testid="stRadio"] div { color: #ffffff !important; font-weight: 500 !important; }
    [data-testid="stSelectbox"] label, [data-testid="stSelectbox"] p,
    [data-testid="stSelectbox"] span { color: #ffffff !important; font-weight: 500 !important; }
    .stCaption, .stCaption p { color: #c8d1dc !important; font-weight: 600 !important; }
    .stTextInput label { color: #c8d1dc !important; font-weight: 600 !important; }
    .stTextInput input {
    ...
    .stTextInput input:focus { border-color: rgba(139,92,246,0.5) !important; box-shadow: 0 0 15px rgba(139,92,246,0.15) !important; }
    ...
    .summary h4 { margin: 0 0 8px 0; color: #34d399; font-weight: 700; }
    ...
    .section-title {
        ...
        color: #a0a8b8; margin-bottom: 12px;
        display: flex; align-items: center; gap: 8px;
    }
    ...
    [data-testid="stCheckbox"] label p, [data-testid="stCheckbox"] p,
    [data-testid="stCheckbox"] span { color: #ffffff !important; font-weight: 500 !important; }
    </style>''',
...
# ---------------- State ----------------
ss = st.session_state
for k, v in [('reset_mods', False), ('reset_all', False), ('mod1', False), ('mod2', False), ('mod3', False), ('last_summary', None), ('cli_lines', []), ('cli_error', None), ('select_all', False), ('pipeline_done', False)]:
...
with left_col:
    ...
    # V4.0: Vendor filter mode
    st.caption('Report Scope')
    scope_col1, scope_col2 = st.columns([1, 1])
    with scope_col1:
        report_scope = st.radio('', [VENDOR_MODE_ALL, 'Vendor Specific'], horizontal=True, key='report_scope', label_visibility='collapsed')
    with scope_col2:
        if report_scope == 'Vendor Specific':
            vendor_choice = st.selectbox('Select Vendor', [VENDOR_MODE_INFY, VENDOR_MODE_OTHER], key='vendor_choice')
        else:
            vendor_choice = VENDOR_MODE_ALL
    vendor_mode = vendor_choice if report_scope == 'Vendor Specific' else VENDOR_MODE_ALL
...
with right_col:
    ...
    # Download log button — only shown after pipeline completes or fails
    if ss.get('cli_lines') and ss.get('pipeline_done'):
        log_text = ''.join(ss['cli_lines'])
        st.download_button('📋 Download Log', data=log_text.encode('utf-8'),
                          file_name=f'dashboard_log_{datetime.now().strftime("%Y%m%d_%H%M%S")}.txt',
                          key='dl_cli_log')
    st.markdown("</div>", unsafe_allow_html=True)
...
if run_btn:
    if not any([m1, m2, m3]):
        ...
        ss['pipeline_done'] = False  # reset until pipeline finishes
            ...
            try:
                    ...
                    with sync_playwright() as p:
                        ...
                        if m1:
                            ...
                            labels_pivot = retry_module(
                                lambda: run_qmetry_automation(browser, iteration, integration, vendor_mode=vendor_mode),
                        ...
                        if m2:
                            logger.set('\U0001F41E Jira Defect Extract + Pivot (Progression) ...')
                            print('[STEP] Running Jira Defect Extract + Pivot (Progression)...')
                            defects_pivot = retry_module(
                                lambda: run_defect_automation(browser, iteration, integration, vendor_mode=vendor_mode, label_type='PROGRESSION'),
                                max_retries=3, delay=5, label='Jira Defects (Progression)')
                            if defects_pivot:
                                results['defects_pivot'] = Path(str(defects_pivot))
                                print('[OK] Progression Defects pivot => %s' % results['defects_pivot'])
                            else:
                                results['defects_pivot'] = None
                                print('[WARN] Progression Defect returned None')
                            # Regression defects
                            logger.set('\U0001F41E Jira Defect Extract + Pivot (Regression) ...')
                            print('[STEP] Running Jira Defect Extract + Pivot (Regression)...')
                            reg_defects_pivot = retry_module(
                                lambda: run_defect_automation(browser, iteration, integration, vendor_mode=vendor_mode, label_type='REGRESSION'),
                                max_retries=3, delay=5, label='Jira Defects (Regression)')
                            if reg_defects_pivot:
                                results['reg_defects_pivot'] = Path(str(reg_defects_pivot))
                                print('[OK] Regression Defects pivot => %s' % results['reg_defects_pivot'])
                            else:
                                results['reg_defects_pivot'] = None
                                print('[WARN] Regression Defect returned None')
                        browser.close()
                ...
                if m3:
                    ...
                    while _m3_attempts < 3:
                        ...
                        try:
                            if m1 and m2:
                                ret = run_qmetry_test_report(iteration, keep_browser=True, vendor_mode=vendor_mode)
                                excel_path, label_excel_path, reg_excel_path, reg_label_excel_path, _qtr_page, _qtr_context, _qtr_browser, _qtr_pw = ret
                            else:
                                excel_path, label_excel_path, reg_excel_path, reg_label_excel_path = run_qmetry_test_report(iteration, keep_browser=False, vendor_mode=vendor_mode)
                            if excel_path:
                ...
                if m1 and m2 and m3:
                    ...
                    # Load regression defects if available
                    reg_dp = results.get('reg_defects_pivot')
                    reg_defects_df = pd.read_excel(reg_dp) if reg_dp and reg_dp.exists() else pd.DataFrame()
                    if not reg_defects_df.empty:
                        reg_defects_df = ensure_release_first(reg_defects_df)
                    ...
                    try:
                        add_slide2_delivery_updates(prs, iteration, labels_df=labels_df, defects_df=defects_df,
                                                    exec_df=qtr_df, reg_exec_df=reg_qtr_df,
                                                    reg_defects_df=reg_defects_df, insert_index=1)
                    ...
                    # V4.0: Current Open Defects summary (2D matrix style)
                    try:
                        if not defects_df.empty:
                            # Build the P0/P1/P2/P3 summary from defect pivot
                            _od_totals = _s2_build_priority_totals(defects_df)
                            _od_grand = sum(_od_totals.values())
                            open_defects_df = pd.DataFrame([{
                                'Service Type': 'T-Mobile',
                                'P0 Blocker/Emergency': _od_totals.get('P0', 0),
                                'P1 Critical/High': _od_totals.get('P1', 0),
                                'P2 Major/Medium': _od_totals.get('P2', 0),
                                'P3 Minor/Low': _od_totals.get('P3', 0),
                                'Total': _od_grand,
                            }])
                            # Add as a styled slide
                            blank = prs.slide_layouts[6]
                            od_slide = prs.slides.add_slide(blank)
                            usable_w = prs.slide_width - Inches(1.80)
                            # Title banner (dark blue)
                            banner_top = Inches(1.50)
                            banner = od_slide.shapes.add_shape(
                                MSO_AUTO_SHAPE_TYPE.RECTANGLE,
                                Inches(0.90), banner_top, usable_w, Inches(0.40))
                            banner.fill.solid(); banner.fill.fore_color.rgb = RGBColor(0, 51, 102)
                            banner.line.fill.background()
                            btf = banner.text_frame; btf.clear()
                            bp = btf.paragraphs[0]
                            bp.text = f'Two Dimensional Filter Statistics: INTG_{iteration}_CURRENT_OPEN_DEFECTS'
                            bp.font.name = FONT_NAME; bp.font.size = Pt(11); bp.font.bold = True; bp.font.color.rgb = WHITE
                            # Table
                            tbl_top = banner_top + Inches(0.50)
                            cols = list(open_defects_df.columns)
                            tbl_shape = od_slide.shapes.add_table(
                                2, len(cols), Inches(0.90), tbl_top, usable_w, Inches(0.70))
                            tbl = tbl_shape.table
                            # Header row
                            for ci, col in enumerate(cols):
                                ...
                                p.font.name = FONT_NAME; p.font.size = Pt(9); p.font.bold = True
                                # Color the P0/P1/P2/P3 labels
                                if 'P0' in col: p.font.color.rgb = RGBColor(204, 0, 0)
                                elif 'P1' in col: p.font.color.rgb = RGBColor(255, 153, 0)
                                elif 'P2' in col: p.font.color.rgb = RGBColor(0, 102, 204)
                                elif 'P3' in col: p.font.color.rgb = RGBColor(102, 102, 102)
                                else: p.font.color.rgb = RGBColor(51, 51, 51)
                            # Data row
                            for ci, col in enumerate(cols):
                                val = open_defects_df.iloc[0][col]
                                cell = tbl.cell(1, ci); cell.text = str(val)
                                cell.text_frame.word_wrap = False
                                p = cell.text_frame.paragraphs[0]
                                p.font.name = FONT_NAME; p.font.size = Pt(10)
                                if col == 'Total':
                                    p.font.bold = True; p.font.color.rgb = RGBColor(0, 51, 102)
                                elif col != 'Service Type':
                                    p.font.color.rgb = RGBColor(0, 102, 204)
                            # Column widths
                            tbl.columns[0].width = Inches(1.50)
                            for ci in range(1, len(cols)):
                                tbl.columns[ci].width = int((usable_w - Inches(1.50)) / (len(cols) - 1))
                            add_brand_footer_logo(od_slide, prs, logo)
                            print('[OK] Current Open Defects slide added')
                    except Exception as od_err:
                        print('[WARN] Open Defects slide failed: %s' % od_err)
...
st.markdown("<div style='text-align:center;padding:12px 0;color:#b8c0cc;font-size:11px;font-weight:500;letter-spacing:0.5px;'>"
            "MDA Jira Dashboard &bull; V5.0 &bull; Aurora Glass</div>", unsafe_allow_html=True)
